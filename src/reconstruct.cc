...
//author: Ara Nefian
std ::vector<Vector3> ReadSunPosition(char *filename, vw::uint32 numEntries)
{
  ...
  for (vw::uint32 i = 0; i < numEntries; i++){
...
//author: Ara Nefian
void ComputeExposureTime(std::vector<Vector4> const& samples,
                         modelParams prevModelParams, modelParams* currModelParams, float *error) {
  ...
  for ( vw::uint32 i = 0; i < samples.size(); ++i ) {
                          ...
                          modelParams currModelParams, modelParams prevModelParams)
{
    ...
    for ( vw::int32 k=0; k < curr_image.rows(); ++k ) {
      for ( vw::int32 l=0; l < curr_image.cols(); ++l ) {
         ...
         if ( is_valid(curr_image(l,k)) ) {
           ...
           if ( is_valid (prev_image_pixel) /*&& (prev_sample_pix[0] >= 0) && (prev_sample_pix[1] >= 0)*/) {
               ...
               albedo = albedo/temp_den;
               //int int_albedo = (int)floor(albedo); // not used
