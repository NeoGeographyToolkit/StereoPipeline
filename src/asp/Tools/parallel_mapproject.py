...
import asp_file_utils, asp_system_utils, asp_cmd_utils

if sys.version_info < (2, 6, 0):
    print('\nERROR: Must use Python 2.6 or greater.')
    sys.exit(1)

# Add the path to the script to the system path, so that GNU Parallel can find it.
path = sys.path[0]
...
def handleArguments(args):
    ...
    for i in iterable:
        ...
        if python_utils.is_command_option(a):      # This is the start of an option.
            optionsList.append(a)  # Record this entry.
            
            if python_utils.is_command_option(n):  # The next entry is the start of another option so this one has no values.
                continue
...
def writeSingleTile(options):
    ...
    python_utils.executeCommand(cmd, suppressOutput=options.suppressOutput)
...
def main(argsIn):
    ...
    try:
        ...
        parser     = python_utils.PassThroughOptionParser(usage=usage, epilog=epilogText) # Use parser that ignores unknown options
        ...
        if len(requiredList) < 1:
            parser.error("Missing path to DEM")
        if len(requiredList) < 2:
            parser.error("Missing path to input image")
    ...
    if (not python_utils.isIsisFile(options.imagePath)) and (not options.nodesListPath):
    ...
    python_utils.createFolder(outputFolder)
    ...
    python_utils.createFolder(tempFolder)
    ...
    numNodes = python_utils.getNumNodesInList(options.nodesListPath)
    
    # We assume all machines have the same number of CPUs (cores)
    cpusPerNode = python_utils.get_num_cpus()
    ...
    commandString = python_utils.argListToString(commandList)
   
    
    # Use GNU parallel call to distribute the work across computers
    # - This call will wait until all processes are finished
    python_utils.runInGnuParallel(options.numProcesses, commandString, argumentFilePath, parallelArgs, options.nodesListPath, True)#not options.suppressOutput)
    ...
    if not options.keep:
        python_utils.removeFolderIfExists(tempFolder)
