...
void createImageMasks(ASPGlobalOptions & opt,
  ...
  if (has_left_georef && has_right_georef && opt.session->isMapProjected()) {
    ...
    // Optionally grow the warped mask before intersecting, so two mapprojected
    // images that are shifted relative to each other (such as from an offset
    // drape) keep their overlap. See --mapproj-geolocation-uncertainty.
...

    {
      ...
      if (geoloc_unc > 0)
        warped_right_mask = asp::expandMask(warped_right_mask, geoloc_unc);
    ...
    }
    {
      ...
      if (geoloc_unc > 0)
        warped_left_mask = asp::expandMask(warped_left_mask, geoloc_unc);
