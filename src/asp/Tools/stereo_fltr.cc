...
// New filtering methods copying the existing format from from vw/Stereo/DisparityMap.h
//###########################################################################################
//###########################################################################################
// REPLACEMENT 2: Just compare the central point to the mean of the neighbors (Ara's method)
  ...
  ///
  /// pixel_threshold     = Absolute value a center pixel can be away from the mean
  /// rejection_threshold = Not used.
  template <class ViewT>
...
// REPLACEMENT 3: Use the standard deviation of the kernel pixels to determine valid range
  ...
  public:
    ...
    typename PixelAccessorT::pixel_type operator() (PixelAccessorT const& acc) const 
    {
      ...
      {
        //vw_out() << "errorX = " << errorX << " errorY = " << errorY << " meanX = " << meanX << " meanY = " << meanY << " xStdDev = " << stdDevX << " yStdDev = " << stdDevY << " m_pixel_threshold = " << m_pixel_threshold << " m_rejection_threshold = " << m_rejection_threshold <<"\n";      
        m_state->rejected_points++;
        return typename PixelAccessorT::pixel_type();  // Return invalid pixel
      }
  ...
  /// You supply the half dimensions of the kernel window.
  ///
  /// pixel_threshold     = Number of standard deviations a center pixel can be away from the mean
  /// rejection_threshold = Minimum standard deviation (preserves pixels in flat areas)
  template <class ViewT>
  inline UnaryPerPixelAccessorView< UnaryPerPixelAccessorView<EdgeExtensionView<ViewT,ConstantEdgeExtension>,
...
// REPLACEMENT 4: Fit a plane to the other pixels and see how well the test pixel fits the plane
  ...
  //TODO: Move these functions to a common code area!
  ...
  public:
    ...
    typename PixelAccessorT::pixel_type operator() (PixelAccessorT const& acc) const 
    {
      ...
      {
        //vw_out() << "errorX = " << errorX << " errorY = " << errorY << " meanX = " << xMean << " meanY = " << yMean << " xStdDev = " << xStdDev << " yStdDev = " << yStdDev << " m_pixel_threshold = " << m_pixel_threshold << " m_rejection_threshold = " << m_rejection_threshold <<"\n";
  ...
  /// You supply the half dimensions of the kernel window.
  ///
  /// pixel_threshold     = Number of standard deviations a center pixel can be away from the fitted plane
  /// rejection_threshold = Minimum standard deviation (preserves pixels in flat areas)
  template <class ViewT>
  inline UnaryPerPixelAccessorView< UnaryPerPixelAccessorView<EdgeExtensionView<ViewT,ConstantEdgeExtension>,
...
// End of hijacked template specialization code
//###########################################################################################
        ...
        if ( stereo_settings().rm_cleanup_passes == 1 )
          ...
          filtered_disparity =
            stereo::disparity_mask(MultipleDisparityCleanUp<input_type,2>()(
                                               ...
                                               stereo_settings().rm_half_kernel.y(),
                                               stereo_settings().rm_threshold,
                                               stereo_settings().rm_min_matches), 
                                   apply_mask(asp::threaded_edge_mask(left_mask,0,mask_buffer,1024)),
                                   apply_mask(asp::threaded_edge_mask(right_mask,0,mask_buffer,1024)));
