...
void produce_lowres_disparity(ASPGlobalOptions & opt) {
  ...
  // Read this to print some text while still in low-res disparity computation mode.
  // Next time we call this it will be per individual tile so it will go to different
  // log files.
...
BBox2 approximate_search_range(ASPGlobalOptions & opt, double ip_scale,
  ...
  if (std::max(search_range.width(), search_range.height()) <= MIN_SEARCH_WIDTH &&
      stereo_settings().max_disp_spread <= 0) {
  ...
  if (stereo_settings().max_disp_spread > 0) {
    ...
    double mid_x = dx[dx.size()/2];
    double mid_y = dy[dy.size()/2];
    
    double len = stereo_settings().max_disp_spread;
    double half = len / 2.0;
    BBox2 spread_box(mid_x - half, mid_y - half, len, len);
    search_range.crop(spread_box);
    
    vw_out() << "Refined search range using --max-disp-spread: "
             << search_range << "\n";
  }
