...
void produce_lowres_disparity( Options & opt ) {
  ...
  if ( stereo_settings().seed_mode == STEREO_SEED ) {
  ...
  }else if ( stereo_settings().seed_mode == DEM_SEED ) {
    ...
    produce_dem_disparity(opt, left_camera_model, right_camera_model,
                          opt.session->name());
  }else if ( stereo_settings().seed_mode == SPARSE_DISP ) {
...
public:
  ...
  inline prerasterize_type prerasterize_helper(BBox2i const& bbox) const {
    ...
    bool use_local_homography = (stereo_settings().corr_mode == LOCAL_HOMOGRAPHY);
    ...
    if (stereo_settings().corr_mode == PYRAMID) {
    ...
    } else if (stereo_settings().corr_mode == LOCAL_HOMOGRAPHY) {
      return stereo::pyramid_correlate
        ( m_left_image,   right_trans_img,
          ...
          stereo_settings().xcorr_threshold,
          stereo_settings().corr_max_levels ).prerasterize(bbox);
    } else if (stereo_settings().corr_mode == MAPPING) {
...
void stereo_correlation( Options& opt ) {
  ...
  if ( stereo_settings().seed_mode == DEM_SEED ||  stereo_settings().seed_mode == SPARSE_DISP ){
    // D_sub_spread is mandatory for seed_mode 2 and 3.
    sub_disp_spread =
      DiskImageView<PixelMask<Vector2i> >(opt.out_prefix+"-D_sub_spread.tif");
  }else if ( stereo_settings().seed_mode == STEREO_SEED ){
  ...
  ImageView<Matrix3x3> local_hom;
  if ( stereo_settings().seed_mode > NO_SEED && stereo_settings().corr_mode == STEREO_SEED){
    string local_hom_file = opt.out_prefix + "-local_hom.txt";
    read_local_homographies(local_hom_file, local_hom);
  }

  stereo::CostFunctionType cost_mode;
  if      (stereo_settings().cost_mode == ABS_DIFFERENCE) cost_mode = stereo::ABSOLUTE_DIFFERENCE;
  else if (stereo_settings().cost_mode == SQUARE_DIFFERENCE) cost_mode = stereo::SQUARED_DIFFERENCE;
  else if (stereo_settings().cost_mode == NCC) cost_mode = stereo::CROSS_CORRELATION;
  else
