...
void handle_arguments( int argc, char *argv[], Options& opt ) {
  ...
  general_options.add_options()
    ...
    ("session-type,t",   po::value(&opt.stereo_session)->default_value(""),
     "Select the stereo session type to use for processing. Choose 'rpc' if it is desired to later do stereo with the 'dg' session. [options: pinhole isis rpc spot5 aster opticalbar]")
