    ...
    vw::ip::InterestPointList process_image(vw::ImageViewBase<ViewT> const& image ) const {
      ...
      for ( int scale = 2; scale < m_scales; scale++ ) {
        ...
        interest_data.push_back( DataT(original_image, integral_image) );
        {
          ...
          Timer t("done, elapsed time", DebugMessage, "interest_point");
          m_interest( interest_data[scale], scale );
        }
        ...
        AccessT l_row = interest_data[scale-2].interest().origin();
        AccessT m_row = interest_data[scale-1].interest().origin();
        AccessT h_row = interest_data[scale].interest().origin();
      ...
      // Actually threshold and run harris
      ip::InterestPointList::iterator pos = new_points.begin();
      while ( pos != new_points.end() ) {
        int scale = pos->scale;
        pos->scale = m_interest.float_scale( pos->scale );
        if ( !m_interest.threshold( *pos, interest_data[scale],
                                        scale ) ) {
