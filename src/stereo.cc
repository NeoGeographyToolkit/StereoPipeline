...
int main(int argc, char* argv[]) {
  ...
  if (entry_point <= PREPROCESSING) {
    ...
    try {
      ...
      std::cout << "Reading camera " << cam_file1 << " as primary camera: \n";
      std::cout << "Reading camera " << cam_file2 << " as secondary camera: \n";
      std::cout << std::endl;    
    } catch (IOErr& e) { 
      std::cout << e.desc() << "\n";
    ...
    if (boost::ends_with(boost::to_lower_copy(cam_file1), ".cahvor") &&
      ...
      write_image(out_prefix + "-R-lin.tif", Rimg);
                ...
                (boost::ends_with(boost::to_lower_copy(cam_file1), ".pin") &&
                 boost::ends_with(boost::to_lower_copy(cam_file2), ".pin")) ) {
