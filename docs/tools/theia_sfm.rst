...
--theia-flags <string (default: "")>
    ...
    The option ``--random_seed=1`` is passed to Theia by default, for
    reproducibility. Note that Theia's structure-from-motion is not fully
    bit-reproducible even single-threaded, so results can vary slightly
    from run to run.
