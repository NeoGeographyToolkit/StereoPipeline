...
If the source point cloud covers a much larger area than the reference
point cloud, the algorithm may fail, again due to the nature of ICP. To
help the tool decide which source points to align with the reference,
one can use the option \texttt{-\/-max-displacement}, and provide it
with a value, say 100 meters. The algorithm will interpret this to mean
that as result of alignment the source points will not move by a larger
amount than this value. It will then discard all points in the input
source point cloud which are further than this value from the reference
point cloud before doing the alignment. The actual displacement of
source points as result of alignment will be displayed after the
alignment finishes, and it can help refine the number to be passed as
...
Lastly, \texttt{pc\_align} displays the maximum displacement between source
...
\texttt{-\/-max-num-source-points \textit{default: 100000}} & Maximum number of (randomly picked) source points to use (after discarding gross outliers). \\ \hline
