...
This functionality can ba accessed by turning on polygon editing from the \textit{Vector Layer} menu, and then right-clicking with the mouse
to acess the various functions. Subsequently, one can use \texttt{gdal\_rasterize} to keep or exclude the portion of a given image/DEM that is within
...
the Fast Global Registration algorithm (\url{https://github.com/IntelVCL/FastGlobalRegistration}),
feature-based alignment (terrains are hillshaded and matches are found among them),
...
using the \texttt{-\/-fgr-options} field (see the table below for more detail).
...
\texttt{-\/-fgr-options} & Options to pass to the Fast Global Registration algorithm, if used. Default: 'div_factor: 1.4 use_absolute_scale: 0 max_corr_dist: 0.025 iteration_number: 100 tuple_scale: 0.95 tuple_max_cnt: 10000'. \\ \hline
