...
\texttt{-\/-output-nodata-value \textit{float}} & 
Set the output nodata value. Only applicable if the output is a single-channel image with pixels that are float or double. The default is the smallest value for the current data type.  \\ \hline
