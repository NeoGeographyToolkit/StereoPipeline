...
\texttt{-\/-help|-h} & Display this help message\\ \hline
...
The \texttt{stereo\_mpi} program is a modification of \texttt{stereo}
designed to distribute the stereo processing over multiple computing
nodes.

It is important to note that \texttt{stereo\_mpi} can, as of now, run
only as a scheduled job on a supercomputer, and it depends on Intel's
\texttt{mpiexec} to distribute the jobs.

Also, note that, when invoking this tool, only stages 1, 2, and 4 of
...
For these reasons, while \texttt{stereo\_mpi} can be called to do all
stages of stereo generation from start to finish in one command, it may
be more reseource-efficient to invoke
it using just one node for stages 0 and 3, many nodes for stages 1 and
2, and just a handful of nodes for stage 4 (triangulation).

\texttt{stereo\_mpi} accepts the following options (any additional
options given to it will be passed to the stereo executables
for each stage).

\begin{longtable}{|l|p{7.5cm}|}
\caption{Command-line options for stereo\_mpi}
\label{tbl:stereompi}
\endfirsthead
\endhead
\endfoot
...
\texttt{-\/-help|-h} & Display this help message.\\ \hline
\texttt{-\/-mpiexec \textit{integer(=1)}} & The number of computing nodes to use. \\ \hline
...
\texttt{-\/-session-type|-t pinhole|isis|dg} & Select the stereo session
type to use for processing (e.g., ISIS, DG, etc.).\\ \hline
\texttt{-\/-stereo-file|-s \textit{filename(=./stereo.default)}} & Define the stereo.default file to use.\\ \hline
\texttt{-\/-entry-point|-e integer(=0 to 5)} & Stereo Pipeline entry point. \\ \hline
\texttt{-\/-entry-point|-e integer(=1 to 6)} & Stereo Pipeline stop point. \\ \hline
...
image tile for a single process. \\ \hline
\texttt{-\/-correlation-timeout \textit{integer(=none)}} & Timeout in seconds for a stereo correlation process.\\ \hline
\end{longtable}
...
The Mars areoid is MOLA MEGDR (\url{http://geo.pds.nasa.gov/missions/mgs/megdr.html}). We adjusted the areoid height values to be relative to the mean Mars radius of 3,396,190 meters used internally in ASP, from the original value of 3,396,000 meters used at that source.
...
Options & Description \\ \hline \hline
\texttt{-\/-help|-h} & Display this help message\\ \hline
\texttt{-\/-nodata-value \textit{integer(=-32767)}} & The value of no-data pixels, unless specified in the DEM \\ \hline
\texttt{-\/-output-prefix|-o \textit{filename}} & Specify the output file prefix \\ \hline
...
and create the appropriate combined camera XML file. The tool needs to be applied
twice, for each of the left and right image sets.
...
It is important to note that Digital Globe camera files contain, in
addition to the original camera models, their RPC approximations
(section \ref{rpc}). As of now, this tool does not attempt to create a
combined RPC model, as such, one cannot map-project the mosaiced images
with the goal of computing stereo from them (section \ref{mapproj}). This
functionality is planned for the next version of ASP.
