...
DEMs, as well as RGB images. In this mode just images need to be passed
as inputs, without cameras or stereo options. \texttt{stereo\_gui}
handles large images by building on disk a pyramid of increasingly
coarser subsampled images for each input image, and displaying the
subsampled versions that are appropriate for the current level of zoom.

If the images are georeferenced, invoking the tool with
\texttt{-\/-use-georef} and \texttt{-\/-single-window} will correctly
superimpose the images and individual images can be toggled on and off.
...
separate Worldview style XML file containing four ground control points.
...
Here we describle ASP's shape-from-shading tool, named
...
\texttt{-n | -\/-max-coarse-iterations \textit{int(=10)}} & How many iterations to do at levels of resolution coarser than the final result.\\ \hline
