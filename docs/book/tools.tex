...
\texttt{-\/-datum WGS_1984|D_MOON|D_MARS|MOLA} & Use this datum to interpret the heights.\\ \hline
\texttt{-\/-t\_srs} & Specify the GeoReference projection (PROJ.4 string).\\ \hline
\texttt{-\/-session-type|-t pinhole|nadirpinhole|isis|dg|rpc|spot5|aster} & Select the stereo
session type to use for processing. Normally this is autodetected. \\ \hline
\texttt{-\/-bundle-adjust-prefix \textit{string}} & Use the camera
adjustment obtained by previously running bundle\_adjust with this
output prefix. \\ \hline
...
D_MARS and MOLA datums).  The tool will try to automatically find
...
extension (\url{http://opencv.org/}https://github.com/OSGeo/proj.4/wiki/HTDPGrids}).
