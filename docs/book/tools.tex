...
The tool outputs the translation component of this transform, defined as
the vector from the centroid of the original source points to the
centroid of the transformed source points. We display this translation
component in three ways (a) Cartesian coordinates with the origin at the
planet center, (b) Local North-East-Down coordinates at the centroid of
the original source points, and (c) Latitude-Longitude-Height
...
75\%, and 100\% of the errors, all these before and the after alignment
...
rotation change at each itereation) is saved to disk, and can be used to
...
visible in the images, but manifest themselves very strongly
in the the DEMs obtained with ASP.
...
\texttt{-\/-xoffset \textit{arg}} &
Specify the CCD offset correction to apply in the x
direction. Default: 0.2842 (forward scan), 0.3396 
(reverse scan).  \\ \hline

\texttt{-\/-yoffset \textit{arg}} &
Specify the CCD offset correction to apply in the y
direction. Default: 0.2369 (forward scan), 0.3725 
(reverse scan).  \\ \hline
\end{longtable}
