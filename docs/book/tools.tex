...
a DEM. (ASP is able to use map-projected images to run stereo, section
\ref{mapproj-example}.)
...
\texttt{-\/-tr}. Ideally it should be very close to the known image resolution
...
The transform obtained by \texttt{pc\_align} is output to a text file as a
...
guess (section \ref{prevtrans}). The inverse transform is saved to a file as well.
...
\texttt{-\/-coarse-levels \textit{int(=0)}} & Solve the problem on a grid coarser than the original by a factor of 2 to this power, then refine the solution on finer grids.\\ \hline
\texttt{-\/-max-coarse-iterations \textit{int(=50)}} & How many iterations to do at levels of resolution coarser than the final result.\\ \hline
...
\texttt{-\/-bundle-adjust-prefix \textit{string}} & Use the camera adjustments obtained by previously running bundle\_adjust with this output prefix.\\ \hline
...
\section{camera-calibrate}
\label{camera-calibrate}

The \texttt{camera-calibrate} tool can generate camera models suitable for use by \texttt{camera-solve}
and other ASP tools.  This tool only solves for intrinsic camera parameters; to obtain the camera pose you should 
use the \texttt{camera-solve} tool.  This tool is a wrapper around the OpenCV \url{http://opencv.org/} checkerboard
calibration tool which takes care of converting the output into readily usable formats.  When you run the tool,
three camera model files will be created in the output folder: 
"solve_cam_params.txt", "vw_cam_params.tsai", and ocv_cam_params.yml.
The first file can be used as a camera calibration file for the \texttt{camera-solve} tool.  The second file
is a pinhole camera format that is recognized by ASP but remember that the extrinsic parameters were not solved for
so ASP is limited in what it can do with the camera file.  The last file contains the camera information as formatted
by the OpenCV calibration tool.
...
You must also specify an image wildcard path such as "checkers/image_*.jpg".  You may need to enclose this
...
\begin{longtable}{|l|p{7.5cm}|}
\caption{Command-line options for camera-solve}
\label{tbl:camera-solve}
\endfirsthead
\endhead
\endfoot
\endlastfoot
...
\texttt{-\/-flash-pictures} & Pop up windows with pictures of the checkerboard to verify 
the image processing is working well.\\ \hline
...
\section{camera-solve}
\label{camera-solve}

The \texttt{camera-solve} tool can generate pinhole sensor models for input images lacking metadata.
This tool is primarily intended to solve for the extrinsic camera parameters.  If you do not provide
intrinsic parameters the tool will try to compute them but do not expect good results.  You can use the
\texttt{camera-calibrate} tool to solve for intrinsic parameters.
In order to register the camera models in world coordinates a set of ground control points
will be needed establishing the real world
coordinates of several points in the images.  The easiest way to generate these points is to use
the ground control point writer tool available in the \texttt{stereo-gui} tool.  The pinhole models
generated by this tool can be used normally with all other ASP tools.

The camera calibration file should be a plain text file with each line corresponding to one input image file.
The format of each line is:
\begin{itemize}
\item file path (or a wildcard path to multiple files using the * symbol)
\item focal length (in pixel units)
\item focal point x (in pixels)
\item focal point y (in pixels)
\item aspect ratio
\item skew
\item radial distortion factor k1
\item radial distortion factor k2
\end{itemize}

This tool will generate two types of output files.  One is a set of .tsai files, one per input image.
These are camera models in a local coordinate system and they are plain text files for easy inspection.
If ground control points are provided the tool will also generate a .pinhole model for each image.
These files contain the world coordinates for each camera.  You can use either file with the other ASP
tools but remember that the .tsai files are only in an arbitrary local coordinate system.  Both sets of
...
\texttt{-\/-calib-file  \textit{string}} & Path to estimated camera parameters.  The tool
works much better if good estimates are provided!\\ \hline
\texttt{-\/-gcp-file  \textit{string}} & Path to a ground control point file.  Without this file,
the tool can only generate camera models in an arbitrary local coordinate system.\\ \hline
\texttt{-\/-bundle-adjust-params  \textit{string}} & Additional parameters (in double quotes)
to pass to the \texttt{bundle-adjust} tool.\\ \hline
...
This tool is a wrapper that relies on on two other tools to operate.  The first of these is
the THEIA software library \url{http://www.theia-sfm.org/index.html} for computing the relative
poses of the cameras.  ASP's \texttt{bundle-adjust} tool is used to register the cameras in
world coordinates using the ground control points.  If the tool does not provide good results
you can customize the parameters being passed to the underlying tools in order to improve
the results.  For \texttt{bundle-adjust} options, see the description in this document.
For more information about THEIA flagfile options see their website.  You can generate a THEIA
flagfile by running the tool; it will generate a default flagfile in the output folder.
