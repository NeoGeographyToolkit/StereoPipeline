...
To make it easier to create interest point matches in situations when the
images are very different or taken from very diverse perspectives, one 
can first mapproject them onto a DEM, as then the images look a
lot more similar. Then interest points are created among the
map-projected images, and they are transferred to the original images.
...
If the mapprojected images are still too different 
for interest point matching to succeed, create them manually in the GUI as follows:

\begin{verbatim}
  stereo_gui A.map.tif B.map.tif C.map.tif run/run
\end{verbatim}

Pick interest points by right-clicking, save them, and quit the GUI. 

Then bundle adjustment is invoked as follows (whether or not interest points were picked in the GUI):
...
This will first create matches among mapprojected images if those do not exist,
then transform those to be among the original images,
and continue with bundle adjustment.
...
Remove as outliers interest points for which the longitude and latitude of the triangulated position (after cameras are optimized) are outside of this range. Specify as: min\_lon min\_lat max\_lon max\_lat.
\\ \hline
