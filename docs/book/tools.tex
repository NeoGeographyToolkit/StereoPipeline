...
be more resource-efficient to invoke
it using just one node for stages 0 and 3, many nodes for stages 1 and
2, and just a handful of nodes for stage 4 (triangulation). As an example, to
call \texttt{stereo\_mpi} just for stage 3, one invokes it with the
options \texttt{-\/-entry-point 3 -\/-stop-point 4}.
...
\texttt{-\/-stop-point|-e integer(=1 to 6)} & Stereo Pipeline stop point. \\ \hline
