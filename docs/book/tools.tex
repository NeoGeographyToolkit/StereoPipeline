...
These can eithre be created by hand, or using \texttt{stereo\_gui}
(section \ref{bagcp}).
...
format already. Then, \texttt{stereo\_gui} can be invoked  to create point
correspondences (interest point matches) from the reference to the
...
from them, can use this transform as an initial guess via the
\texttt{-\/initial-transform} option. If it is desired to use it as it
is, without further adjusting it using ICP, one can specify
\texttt{-\/-num-iterations 0}.
...
WV01 TDI 8 forward scan direction is supported. These are by far the
...
intrinsic parameters the tool will try to compute them but do not expect good results.
