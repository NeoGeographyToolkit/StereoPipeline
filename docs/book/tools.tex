...
You can create ground control point files for bundle_adjust using stereo_gui.
To do this, you must have at least three images loaded and the last of these
images must have a valid georeference.  You must then manually create interest 
point matches between the images.  When you are finished creating 
interest points, use the "IP matches"->"Write GCP file" menu item to generate
a ground control point file containing the selected points.  You will be prompted
for a reference DEM and for the desired output path.  The last image is
only used for it's georeference information to index into the DEM; the 
selected pixels from the last image are not saved to the GCP file.
...
\texttt{-\/-datum \textit{string}} & Set the datum. This will override manually set datum information. Options: WGS\_1984, D\_MOON (1,737,400 meters), D\_MARS (3,396,190 meters), MOLA (3,396,000 meters), NAD83, WGS72, and NAD27. Also accepted: Earth (=WGS\_1984), Mars (=D\_MARS), and Moon (=D\_MOON). \\ \hline
...
format already. Then, \texttt{stereo\_gui} can be used to create point
...
\texttt{pc\_align} can be invoked with the reference and source DEMs and
...
are float64 or the user has specified the float64 option. It can merge clouds with
1, 3, 4, or 6 bands.
