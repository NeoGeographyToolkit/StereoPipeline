...
Also writes out a standalone camera model file instead of adjust files. \\ \hline
...
The \texttt{camera\_solve} tool can generate pinhole sensor models (frame
cameras) for input images lacking metadata.  This tool is primarily
intended to solve for the extrinsic camera parameters.  If you do not
provide intrinsic parameters the tool will try to compute them but do
not expect good results.  You can use the \texttt{camera\_calibrate} tool
to solve for intrinsic parameters.

The tool is based on the THEIA software library
\url{http://www.theia-sfm.org/index.html}, which we bundle with ASP.

In order to register the camera models in world coordinates a set of
ground control points will be needed establishing the real world
coordinates of several points in the images.  The easiest way to
generate these points is to use the ground control point writer tool
available in the \texttt{stereo-gui} tool.  The pinhole models generated
by this tool can be used normally with all other ASP tools.
...
The K1 and K2 terms in the calibration file are used as follows:\\
\hspace*{2em}\texttt{r2 = x*x + y*y} \\
\hspace*{4em}\texttt{d  = 1 + r2*(k1 + k2*r2)} \\
\hspace*{6em}\texttt{x\_distorted = x * d} \\
\hspace*{8em}\texttt{y\_distorted = y * d}
...
options see their website.  You can generate a THEIA flagfile by running
the tool; it will generate a default flagfile in the output folder.
