...
the original values (only for the Ceres solver).  A lower weight means that the values will
change less, a higher weight means more change.
\\ \hline
...
the csv-format setting must also be set.\\ \hline
...
measured from planet center. TODO FILE MATCH\\ \hline
...
cameras) for input images lacking metadata.  Currently the tool only
 solves for the extrinsic camera parameters and it is up to the user to
 provide intrinsic camera parameters.  You can use the \texttt{camera\_calibrate} 
tool to solve for intrinsic parameters.

The tool makes use of the the THEIA software library to generate initial 
camera position estimates in a local coordinate space.  You can learn more 
about THEIA at \url{http://www.theia-sfm.org/index.html}.
...
compatible with ASP.  TODO: LINK TO PINHOLE CHAPTER

This tool will generate two .tsai camera model files in the output folder per input image.
The first file, appended with .tsai, is in a local coordinate system.  The first file also does not 
have the final bundle adjustment applied but it may be useful to aid in debugging strange results.  
