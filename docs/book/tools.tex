...
\texttt{-\/-robust-threshold \textit{float(=0.5)}} & Set the threshold for robust
...
\texttt{-\/-semi-major-axis \textit{float}} & Explicitly set the datum semi-major axis
in meters (needed only if ground control points are used).\\ \hline
\texttt{-\/-semi-minor-axis \textit{float}} & Explicitly set the datum semi-minor axis
in meters (needed only if ground control points are used).\\ \hline
...
\texttt{-\/-max-iterations \textit{integer(=100)}} & Set the maximum number of 
iterations. \\ \hline

\texttt{-\/-lambda \textit{float}} & Set the initial value of the LM parameter 
...
(\texttt{-\/-dem-spacing}). If the output DEM turns out to be sparse,
the output resolution could be increased, or one could experiment with
increasing the value of \texttt{-\/-search-radius-factor}, which will
fill in small gaps in the output DEM by searching further for points in
the input clouds.

It is expected that LAS files have spatial reference information, such
as WKT data. Otherwise it is assumed that the points are raw $x,y,z$
values in meters in reference to the planet center.
 
Unless the output projection is explictely set when invoking \texttt{point2dem},
...
The program \texttt{dem\_mosaic} takes as input a list of DEM files,
optionally erodes pixels at the DEM boundaries, and creates a mosaic,
blending the DEMs where they overlap.
...
The output mosaic tiles will be named <output prefix>-tile-<tile index>.tif,
where <output prefix> is an arbitrary string. For example, if it is 
set to \texttt{results/output}, all the tiles will be in the
\texttt{results} directory. 

By the default, the output mosaicked DEM will use the same grid size and
projection as the first input DEM. These can be changed via the
...
Options & Description \\ \hline \hline
\texttt{-\/-help|-h} & Display the help message.\\ \hline
\texttt{-\/-reference-spheroid|-r earth|moon|mars} & Set the reference spheroid. This will create a geo-referenced las file in respect to the spheroid. \\ \hline
\texttt{-\/-compressed} &
...
cloud format, DEMs as GeoTiff or ISIS cub files, or plain-text CSV files (with .csv
...
Digital Globe images, that is, before calling \texttt{dg\_mosaic} and
\texttt{mapproject}. 
