...
and turn on viewing of interest point mathes to study if they were
...
D\_MARS and MOLA datums).  The tool will try to automatically find
...
\texttt{-\/-keep-bounds} option is not set there may be noticable changes in
...
critical that you have a very accurate conversion we reccomend that you attempt 
...
ASP is able to read these files to recover the IPs.  In addition to generating these files, 
\texttt{ipfind} is a good tool to test out different IP detection settings using the output debug image.

You can pass multiple input images to the tool and they will be processed one after the other.

Usage:
\begin{verbatim}
  ipfind [options] <image> ...
\end{verbatim}
...
Options & Description \\ \hline \hline
\texttt{-\/-help|-h} & Display the help message. \\ \hline
\texttt{-\/-interest-operator \textit{string}} & Choose an interest point metric from [IAGD (default), LoG, Harris, OBALoG, Orb, Sift]. \\ \hline
\texttt{-\/-descriptor-generator \textit{string}} & Choose a descriptor generator from [patch, pca, sgrad, sift, sgrad2, orb]. The sift and orb types cannot be mixed.\\ \hline
\texttt{-\/-max-points \textit{integer}} & Set the maximum number of IP per tile (default 250). \\ \hline
\texttt{-\/-gain \textit{float}} & Increasing this number will increase the gain at which interest points are detected. Does not apply to Orb and Sift. (default 1.0). \\ \hline
...
\texttt{-\/-no-orientation } & Turn off rotation invariance. \\ \hline

\texttt{-\/-output-folder \textit{string}} & Write output folders to this location. \\ \hline
\texttt{-\/-debug-image \textit{integer}} & 0 = No debug images (default), 1 = Small debug images, 2 = full size debug images. \\ \hline
\texttt{-\/-print-ip \textit{integer}} & Print information for this many detected IP.\\ \hline
\texttt{-\/-per-tile-normalize} & Normalize each processing tile seperately.\\ \hline
\texttt{-\/-nodata-radius} & Remove IP within this distance of nodata and image boundaries.\\ \hline
\texttt{-\/-lowe} & Save results in an ASCII text format compatible with the Lowe-SIFT toolchain.\\ \hline
\texttt{-\/-tile-size} & The tile size used to split up the image.\\ \hline
\texttt{-\/-num-threads} & Number of simultaneous thread to use for computation.\\ \hline
\end{longtable}
...
\texttt{-\/-matcher-threshold \textit{float}} & Threshold for the seperation between closest and next closest interest points. Default 0.6\\ \hline
