...
(in future this will be expanded to using a DEM).
...
If a ground truth lidar file is present, say named \texttt{lidar.csv}, it can be used
...
\begin{verbatim}
  bundle_adjust -t nadirpinhole --local-pinhole --solve-intrinsics               \
    ...
    --lidar-file lidar.csv --disparity-list run_stereo/run-unaligned-D.tif       \
    -o run_ba_intr_lidar/run
\end{verbatim}

This will write some residual files of the form 
\begin{verbatim}
  initial_residuals_no_loss_function_lidar.txt
  final_residuals_no_loss_function_lidar.txt
\end{verbatim}
