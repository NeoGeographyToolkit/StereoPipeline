...
dangerous tool as careful consideration is required to insure and
...
\citet{lunokhod:controlnetwork} and \citet{lunokhod:gcp}. These tools
...
). That makes it extremely hard to project a random point into the
camera's original model. The math would follow the transformation from
projection into the camera frame, then projected back down to surface
that ISIS uses, then finally up into the infinitely large
sensor. \texttt{Jigsaw} does not support map projected imagery.
...
measurements. For this example, write the following text-box into a
...
are the arguments for \texttt{pointreg}.
...
then open \textit{cube.lis} and then
...
finished, go back to the main Qnet window. Save the final control as
\textit{control\_qnet.net} by clicking on \textit{File}, and then
\textit{Save As}.
