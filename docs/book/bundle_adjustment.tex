...
and run \texttt{stereo\_tri} with the additional option 
\begin{verbatim} 
...
and then on the second and the third, many of the generated interest
points will be seen in all three images in the area of overlap, which
...
\begin{verbatim}
  bundle_adjust -t nadirpinhole --create-pinhole-cameras           \
    --solve-intrinsics --camera-weight 100                         \
...
need to be increased if perhaps it over-fits.

Next, one can run bundle adjustment with fixed intrinsics, to re-adjust
the extrinsics, and then run stereo as before, with the new cameras, and
...
on the ASP-created aligned DEM and the reference lidar/DEM file). 
...
second images, third and fourth, etc., invoke the above logic for each
pair, that is, run stereo, alignment to the ground truth, dense interest
point generation, and transforming the cameras using the alignment
transform matrix.  Then, a directory can be created in which one can
...
\begin{verbatim}
  bundle_adjust -t nadirpinhole --create-pinhole-cameras                                  \
    ...
    --disparity-list 'run_stereo12/run-unaligned-D.tif run_stereo34/run-unaligned-D.tif'  \
    --max-disp-error 10 --max-num-reference-points 1000000 -o run_ba_intr_lidar/run
\end{verbatim}

This methodology suggests that the tool must be invoked with an even
number of images, and there should exist a generated unaligned disparity
for the first and second images, third and fourth, etc, and the
disparity names can be specified as a list in quotes with space as a
separator to the option \texttt{-\/-disparity-list}. The option
...
An example showing how to convert a camera model to RPC is given in section \ref{convertpinholemodel}.
