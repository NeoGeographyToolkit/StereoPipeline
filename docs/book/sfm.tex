...
positions and longitude and latitude of the corners (the heights above datum can be set to zero if not known). Here is a sample file, where the image is named
\texttt{img.tif} (below the latitude is written before the longitude).
...
which will write the desired correctly oriented camera file. 
