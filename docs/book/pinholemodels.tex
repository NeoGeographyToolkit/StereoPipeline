...
ASP supports the panoramic camera for declassified CORONA KH-4B images. It implements the model
...
and the geometric center (midpoint) of the image, in milliliters. The
pixel pitch and focal length \texttt{f} are also in millimeters. The
...
center \texttt{iC} is in meters, and the rotation matrix \texttt{iR} is
represented as the axis angle.
