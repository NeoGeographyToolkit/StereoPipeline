...
\begin{figure}[h]
\centering
  \subfigure[{\tt Framing Camera Model}]{\includegraphics[width=3in]{images/correlation/pinhole_model.png}\label{fig:framing}}
  \subfigure[{\tt Pushbroom Camera Model}]{\includegraphics[width=3in]{images/correlation/linescan_model.png}\label{fig:linescan}}
...
ASP has basic support for multi-view stereo. The first image is set as
reference, disparities are computed from it to the other ones, and joint
triangulation is performed (using the method from
\cite{slabaugh2001optimal}). A single point cloud is output, with one 3D
point for each pixel in the first image. The inputs to multi-view stereo
and its output point cloud can be handled in the same way as for
two-view stereo (e.g., inputs can be map-projected, the output can be
...
file, is computed as twice the mean of distances from the optimally computed
intersection point to the individual rays. For two rays, this agrees
with the intersection error for two-view stereo, which is defined
...
Alternatively, the \texttt{parallel\_stereo} tool can be used (section \ref{parallel}).