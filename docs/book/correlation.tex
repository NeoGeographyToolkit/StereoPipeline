...
search, similar to what is done in the NG-fSGM algorithm described in 
...
to what is used in the tSGM algorithm described in \citep{rothermel2012sure_isgm}.  With
...
In addition to increasing the number of correctly matched pixels, this feature can 
avoid the need for a seperate hole filling step if a gapless DEM is desired.  
This is particularly useful when generating an initial DEM to use for map 
projection of inputs in preparation for a second stereo pass.
SGM also produces finer results than the standard correlation algorithm, meaning that
you may be able to skip running a separate subpixel refinement step 
(use \texttt{subpixel mode 0}).  The primary disadvantage to using SGM (besides the
speed and memory requirements) is that it can cause noticable artifacts at tile boundaries.
We hope to eliminate this issue soon but in the mean time you can mitigate it by
using as large a tile size as possible.

In order to use the SGM algorithm you must take the following steps
when you perform integer correlation:

\begin{itemize}
\item Pass in the \texttt{use-sgm} command line flag to enable the SGM algorithm.
\item Choose a supported kernel size.  Currently the only sizes supported are 3 and 5.
\item Choose an appropriate tile size using the \texttt{corr-tile-size} command. The SGM
algorithm works best on larger tiles so this parameter should be set to as large as your
available RAM can support.  You should generally shoot for a value where the amount of
