...
depending on how you plan to use the DEM you may be able to skip running the subpixel refinement 
step (use \texttt{subpixel mode 0}).  Along with these advantages come several disadvantages.
...
parameter to a negative value.  Disabling this check nearly halves the (long) run time of the SGM
algorithm so consider if the filtering it provides is worth the increased run time.
\item Set \texttt{subpixel-mode} to zero.  The SGM performs parabola subpixel interpolation as part of
the \texttt{stereo\_corr} step and does no work in the \texttt{stereo\_rfne} module.
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=3.0in]{images/correlation/icebridge_input_crop.png}
  \includegraphics[width=3.0in]{images/correlation/icebridge_subpix3_crop.png}
  \includegraphics[width=3.0in]{images/correlation/icebridge_sgm_crop.png}
  \caption{A pair of hill-shaded DEMs from NASA IceBridge images showing the difference between default ASP processing and
  processing using the SGM algorithm.  The DEM on the left was generated using the default stereo parameters and 
  \texttt{subpixel-mode 3}.  The DEM on the right was generated using the following options: 
  \texttt{--use-sgm --threads 1 xcorr-threshold -1 --corr-kernel 7 7 --corr-tile-size 6400 --cost-mode 4 --subpixel-mode 0 
  --median-filter-size 3 --texture-smooth-size 13 --texture-smooth-scale 0.13}.}
