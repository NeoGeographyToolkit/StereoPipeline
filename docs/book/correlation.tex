...
thread count to accelerate processing.
...
Semi-Global Matching also requires special care when processing large images.  Since the results in each pixel
...
\item Set all other SGM parameters as described above.
\end{itemize}
...
affecting the stereo results.  MGM is currently limited to 8 threads.  Eventually these steps will be streamlined
