...
When using SGM in is important to choose an appropriate tile size using the 
\texttt{corr-tile-size} command.  The SGM algorithm works best on larger tiles so this parameter should be set as large as your available RAM can support or larger than the image you wish to process.  
The exact amount of RAM used varies depending on the image content and cannot be exactly predicted in advance so some trial and error may be required.  When running SGM, ASP will process only one
tile at a time but it will use multiple threads per tile.  MGM is currently limited to using
8 simultaneous threads.

Semi-Global Matching requires special care when processing large images.  Since the results in each pixel
are affected by the rest of the image, processing a large image in separate tiles in the usual ASP fashion
can lead to seam artifacts along tile borders.  You may be able to remove these artifacts entirely just by using
the \texttt{sgm-collar-size} option but it may not be sufficient.  A more powerful option is to use the 
\texttt{parallel\_stereo} program instead of the \texttt{stereo} program.  \texttt{parallel\_stereo} replaces
the refinement stage with a new seam blending stage to suppress artifacts along tile borders.  In order to 
properly use \texttt{parallel\_stereo} with SGM, use the following guidelines:

\begin{itemize}
\item Specify the \texttt{collar-size} option or leave it at the default value.
\item Set the \texttt{corr-tile-size} option just as you would when using \texttt{stereo}.  Keep in mind that
the collar size you selected will enlarge the processed tile size.
\item Set \texttt{job-size-w} and \texttt{job-size-h} exactly equal to \texttt{corr-tile-size}.
...
affecting the stereo results.  Eventually these steps will be streamlined
 but for now it is up to the user to ensure that things are set properly.

When SGM or MGM is specified, certain other parameters have their default values
...
algorithm.  Set \texttt{xcorr-threshold} to >= 0 to turn it back on.
...
values are enabled by default and must be manually disabled.
\item The \texttt{prefilter-mode} setting is ignored when using SGM.
\item The \texttt{subpixel-mode} setting is ignored when using SGM.  The SGM performs parabola subpixel interpolation as part of
