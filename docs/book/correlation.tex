...
Keep in mind the following parameter guidelines when using SGM:
...
\item Decide whether to use the census cost mode (\texttt{cost-mode 3}) or to use
mean absolute distance (MAD) (any other value).  The census mode tends to perform better
 overall but can produce artifacts on featureless terrain, in which case MAD works better.  
\item Choose a supported kernel size.  SGM kernels must always be symmetric.
The MAD cost mode can be used with any odd kernel size but the census cost mode 
can only be used with kernel sizes 3, 5, and 7.  When using the MAD cost mode consider
using a kernel size of 1 as it can produce good results.
...
$200*\texttt{tile\_size}^2$ bytes of RAM.
