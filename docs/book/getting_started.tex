...
\emph{MJB: why is it called P19??  What processing was done to go from PDS imagery to the P19 .cub files?  This whole sample data set probably needs refreshing.}
...
overlap ($\sim80$\% is ideal) are the best suited for creating
stereo image pairs. Depending on the characteristics of the mission
data set and the individual images, the degree of acceptable variation
...
program. That being said, for images which contain large
topographic variation (and therefore large disparity differences
across the scene--think Valles Marineris), sometimes map-projecting
the images can help, as it sometimes improves the correlation.
...
well, and if there are photometric issues with the images, those
photometric issues could be mis-interpreted as topography.

They must also be converted to ISIS 3 \texttt{.cub} files.

\emph{MJB: Must?  Probably `should.'}
...
The following example will utilize images from the example MOC dataset, P19.
