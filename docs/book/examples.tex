...
\section{Generic Pinhole Camera}

To best demonstrate the ability of the Ames Stereo Pipeline to process a
generic pinhole camera, we use images from the Apollo 15 Metric camera.  The
...
we need.  Using the data sheets available 
\href{http://apollo.sese.asu.edu/SUPPORT_DATA/AS15_SIMBAY_SUMMARY.pdf}{here}
we can find the lens distortion parameters for metric camera.  Looking at the 
ASP lens distortion models in appendix \ref{chapter:pinholemodels}, we see that this is the Brown-Conrady
model in ASP.  Using the example in the appendix we can fill out
the rest of the sensor model file so it looks as follows:
...
pictures there are black triangles at the center positions but they line up nicely
with the center of the image.  Before we try to solve for the camera positions, we can 
perform a sanity check on the lens distortion parameters.
...
We should get some camera models and see a printout of the final bundle adjustment error
among the program output information:
\begin{verbatim}
Cost:
...
points (GCPs) which can be identified in the pinhole images.  This will allow an accurate 
...
\caption{Solved for camera positions plotted using orbitviz and the resulting DEM color-shaded and overlaid
over a narrow LRONAC DEM.}
\label{fig:pinhole-a15-result-image}
\end{figure}


Another, more difficult, option to move the camera models is to run stereo with the local
camera models, then use the \texttt{pc\_align} tool to align the output point cloud with an
existing DEM that covers the same region.  This method is accurate but may not work well
if the existing DEM is sparse or the terrain is flat.  Instructions for using the
 \texttt{pc\_align} tool are in appendix \ref{pcalign}.

Finally, ASP supports the method of initializing the \texttt{camera\_solve} tool with estimated
camera positions.  This method will not move the cameras to exactly the right location but
it should get them fairly close, hopefully close enough to be used as-is or to be refined using
\texttt{pc\_align} or some other method.  To use this method, pass additional bundle adjust parameters to 
\texttt{camera\_solve} similar to the following line:
...
\section{IceBridge DMS Camera}

The DMS (Digital Mapping System) is a frame camera flown on as part of the NASA IceBridge
program to collect digital terrain imagery of polar and antarctic terrain.  To process this 
data the steps are very similar to the steps described for a generic pinhole camera but there
are some aspects which are particular to IceBridge.
...
This may force you to use the third option listed in the generic pinhole section above which is
to use estimated camera positions to convert the local camera models into global coordinates.
...
IceBridge flight at \url{http://asapdata.arc.nasa.gov/dms/missions.html}.  When using this
method be aware of the \texttt{-\/-camera-weight} option for \texttt{bundle\_adjust}.  If
your cameras being moved too far or too little from the estimated positions you can raise or
lower this parameter respectively to adjust the output of the solver.
...
which are actually close enough to overlap.  

A final tip for processing IceBridge data is that LVIS lidar data available on some flights
can be used to register DEMs created using DMS imagery.  The lidar data comes in plain text 
files that \texttt{pc\_align} can parse using the following option:
...
produce a low-error fit with the narrow lidar tracks.

\begin{figure}[h!]
\centering
  \includegraphics[width=5.5in]{images/examples/pinhole/icebridge_orbitviz_display.png}
\caption{Display of \texttt{camera\_solve} results for ten IceBridge images using \texttt{orbitviz.}}
