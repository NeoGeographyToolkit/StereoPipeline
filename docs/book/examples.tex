...
the Ames Stereo Pipeline. Yet it is also a shortened guide that shows
the commands and stereo.default files used to process data. It can be
hard to try and figure out what settings to start with and hopefully
this will provide starting point ideas.
...
across the scene, \emph{e.g. Valles Marineris}), sometimes
map-projecting the images can help, as it reduces the search range for
the correlator. Map projecting for HiRISE images especially helps
out. The reason is that for Mars, Isis has a rough DEM created from
MOLA data that it projects the data on to. This removes the large
disparity differences between HiRISE images and leaves only the small
detail for the Ames Stereo Pipeline to work on. This method is
acceptable because Isis can work backwards through a map-projection,
back to the original camera model.

The images should also be photometrically calibrated (in whatever fashion
...
This is a proof of concept showing off the strength of building on top
of ISIS. Cassini data is not a goal of the Ames Stereo Pipeline
developers but it's camera model does exist in ISIS.

Getting stereo pairs from satellites that do not orbit their target is
a difficult challenge. From our experiences we have found that we
usually have to settle with images that are not ideal. This means we
attempting stereo pairs with different lighting, little perspective
change, and generally too far away to see parallax. So far we have had no success with
such difficult data like Cassini's, yet it doesn't hurt to try.

\subsection{Rhea}

Rhea is the second largest moon of Saturn and is roughly 1/3rd the
size of our own Moon. In this example, the top right of both images
show a gaint impact basin named Tirawa that is 220 miles across. The
bright white area south of Tirawa is ejecta from a new crater and
poses a difficult challenge for our correlator. In this examples the
Tirawa impact can barely be made out in the 3D data while the new
crater and ejecta become only noise.
...
This a proof of concept showing off the strength of building on top of
