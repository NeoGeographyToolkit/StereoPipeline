...
\begin{verbatim}
    > ln -s  METADATA.DIM front/SEGMT01/METADATA_FRONT.DIM
    > ln -s  METADATA.DIM back/SEGMT01/METADATA_BACK.DIM
...
\begin{verbatim}
  ...
  stereo -t rpc --subpixel-mode 2 out-Band3N_proj.tif out-Band3B_proj.tif \
...
ASP also implements the exact ASTER linescan camera model, in addition
to the RPC approximation. To take advantage of that, in the commands above
one can run stereo with the \texttt{aster} session instead of \texttt{rpc}.
The \texttt{mapproject} command should not be modified. Just as for
Digital Globe images, it is sufficient to use the approximate RPC model
in map-projection even when using the exact linescan model in the stereo
step.

We did not get better results by using the exact linescan model as
opposed to the RPC model when compared to pre-existing ASTER DEMs. Using
ASP's \texttt{bundle\_adjust} as a pre-processing step did not improve
the results either.
