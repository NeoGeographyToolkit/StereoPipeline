...
The Community Sensor Model, established by the U.S. defense and
...
\citep{hare2017community}. ASP supports for CSM planetary data via the USGS
ISIS implementation (\url{https://github.com/USGS-Astrogeology/usgscsm})
that we ship with our software. 

What follows is an example of using this sensor model for Mars imagery,
specifically for the CTX camera. The images are regular \texttt{.cub}
files as in the tutorial in chapter \ref{ch:moc_tutorial}. Yet the camera
information is this time stored separately in plain text \texttt{.json} files
that contain various metadata for this sensor, including ephemeris time, focal length,
linescan rate, etc. for this particular linescan sensor. 
