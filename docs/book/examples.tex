...
\begin{verbatim}
  bundle_adjust v[1-4].tif v[1-4].tsai v[1-4].gcp -o ba/run -t nadirpinhole       \
   ...
   --overlap-limit 4 --create-pinhole-cameras --camera-weight 0                   \
...
If the baseline of images is small, so one image covers roughly the same footprint 
as some other one in the sequence, likely nothing will help and different images should be used.

Assuming that the cameras turn out well, and that alignment succeeds (whether automatically
or manually), one can apply the alignment transform to the obtained cameras to make them consistent
with the reference DEM. That can be done as follows:

\begin{verbatim}
  bundle_adjust v[1-4].tif  ba/run-v[1-4].tsai -o ba_align/run                   \
   -t nadirpinhole --max-iterations 0 --overlap-limit 1 --create-pinhole-cameras \
...
\begin{verbatim}
  ...
  --overlap-limit 1 --create-pinhole-cameras --robust-threshold 2                   \
  --force-reuse-match-files --max-disp-error 100
\end{verbatim}

If invoking this creates new match files, it means that the dense match files 
were not copied successfully to the new location. 
...
to some camera model having these (see chapter \ref{chapter:pinholemodels}), 
