...
to remove the most of empty area around the image.  This is a good opportunity to 
...
the terrain model made from them, we will a ground truth source. 
...
between launches of the KH4 satellite.  You will need to experiment to see which combination of settings
...
update image\_size, image\_center, and the pitch to account for the downsampling.
...
Now we create a sample camera model for the KH7 using known information about the satellite.
...
are scan\_rate and speed.  The KH7 satellite only contains a single camera so the
...
The KH9 satellite contained one frame camera and two panoramic cameras,
...
Processing this data is similar to processing KH4B data except that the 
...
As with KH4B, this satellite contains a forward pointing and aft pointing camera that need to have
...
As with KH7, it is best to first experiment with low resolution copies of the images.
...
From this point KH9 data is processed in a very similar manner to the KH4B and KH7 examples.
