...
\item[datum \textnormal (default = WGS\_1984)] \hfill \\ 
Set the datum to use with RPC camera models. Options: WGS\_1984, D\_MOON (1,737,400
...
A higher factor will result in more interest points, but perhaps also
...
\item[corr-seed-mode \textnormal{\small{(=0,1,2,3)}}] (default = 1) \hfill \\
  ...
  \item[2 - Low-resolution disparity from an input DEM] - Use a
    ...
    \texttt{disparity-estimation-dem-error} respectively.  This option
    is not compatible with map projected input images.
...
\item[remove-outliers-by-disparity-params \textnormal{\small{(\emph{double double})}} (default = 100 3)]  \hfill \\
...
\item[phase-subpixel-accuracy \textnormal{\small{(\emph{integer})}} (default = 20)]
  Set the maximum resolution of the phase subpixel correlator.
  The maximum resolution is equal to 1.0 / this value.  Larger
