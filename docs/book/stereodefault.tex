...
\item[rm-quantile-multiple \textnormal{\small{(\emph{double})}} (default = -1)] \hfill \\
  ...
  greater than MULTIPLE*PERCENTILE will be discarded.  If this value is set greater
  than zero, this filtering method will be used instead of the
  method using the values RM\_MIN\_MATCHES and RM\_THRESHOLD.
  This method will help filter out small clusters of pixels which are too large to be
  filtered out by the neighborhood method, but care must be taken or it will filter out
  small but valid sections of image.
