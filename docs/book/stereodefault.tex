...
\item[cost-mode \textnormal{\small{(= 0,1,2)}}] (default = 2) \hfill \\
  ...
  difference.  The census transform \citep{zabih1994census} can 
  only be used with the SGM correlator.
...
\item[use-sgm \textnormal (default = false)] \hfill \\

  This flag, if provided, enables using the semi-global matching algorithm instead
  of the normal ASP integer correlator.  See section \ref{sec:sgm} for details.
...
\item[corr-tile-size \textnormal{\small{(\emph{integer})}} (default = 1024)]\hfill \\
  ...
  when using semi-global matching. See section \ref{sec:sgm} for details.
