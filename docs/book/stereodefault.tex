...
\item[min-valid-triangulation-error \textnormal{\small{(= \emph{float})}}]
\hfill \\

Points with triangulation error (closest distance between rays in
triangulation) larger than this are removed from the cloud by being set
to an invalid value (the center of the planet).
