...
will find a directory titled \texttt{052783824050\_01\_P001\_PAN}. It has a
...
turns out to be noisy or have holes, one could change the search radius
factor, use hole-filling, more aggressive outlier removal, and erosion
of pixels at the boundary (those tend to be less
reliable). Alternatively, holes can be filled with \texttt{dem\_mosaic}.
...
you know that your input images are about 1 meter/pixel, use the same
number in \texttt{mapproject} (of course, if your projection is in
degrees, you should convert this value to degrees before using it). If
the output resolution is not correct, there will be artifacts in the
stereo results.
...
and M1121209902LE from ASU's LRO NAC web site, http://lroc.sese.asu.edu/. We convert them
to ISIS cubes using the ISIS tool \texttt{lronac2isis}, then we use the
ISIS tools \texttt{lronaccal}, \texttt{lrnonacecho}, and
\texttt{spiceinit} to do radiometric and echo correction and update the
SPICE kernels. We name the two obtained .cub files \texttt{left.cub} and
\texttt{right.cub}.
...
For non-Digital Globe images we could have used the same process but
we'd invoke the \texttt{rpcmaprpc} session during stereo. In fact, this
is possible for Digital Globe imagery as well, in that case we'll simply
bypass the accurate linescan model altogether and use the RPC model at
all stages of the process.

\subsubsection{Pinhole Camera Models}

Map-projected images can also be used with Pinhole camera models. The
\texttt{mapproject} command needs to be invoked with \texttt{-t
pinhole}. When invoking \texttt{stereo}, the first two arguments
should again be the images, followed by the camera models, output
prefix, and the name of the DEM used for map-projection. The session
name passed to \texttt{stereo} should be either
\texttt{pinholemappinhole} or \texttt{pinhole} (the tool is able to
understand that, since an input DEM was passed in, both of these refer to
stereo with map-projected images).
...
\item Use the GUI (section \ref{stereo_gui}) to get comfortable with ASP on a
  small region and to tune parameters (section \ref{stereo_gui}).
...
from planet center or camera center can be removed as outliers
