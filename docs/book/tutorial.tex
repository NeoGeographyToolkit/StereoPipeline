...
The string \texttt{stereo-output} is an arbitrary output prefix, 
it is used when generating names for \texttt{stereo} output files. For
example, it can be set to \texttt{results/output}, in which case all output
files will be in the \texttt{results} directory and start with the
prefix \texttt{output}.
...
In this example we use as input low-resolution DEM the file
...
first band and ignore the rest. If desired to use one of the other
bands, it can be first singled out by invoking \texttt{dg\_mosaic}
(section \ref{rawdg}) with the \texttt{-\/-band <num>} option. We have
evaluated using ASP with Digital Globe's multi-spectral images, yet
...
correlated simultaneously across multiple images. It offers however,
...
Given several images, at the simplest, pairwise stereo with generation
of multiple point clouds and DEMs can be performed (section
\ref{builddem}), then the DEMs can be combined with \texttt{dem\_mosaic}
(section \ref{demmosaic}).

Alternatively, multiple point clouds can be created, and then a single
DEM can be output with \texttt{point2dem} (section \ref{point2dem}).
...
images. To this approach we will devote the rest of this section. In
this scenario, the first image is set as reference, disparities are
computed from it to the other ones, and joint triangulation is
performed, using the method from \cite{slabaugh2001optimal}. A single
point cloud is output, with one 3D point for each pixel in the first
...
sequence can be used as a reference (so needs to be passed to the tools
as the first image).

The rays intersection error, that is, the fourth band in the point cloud
...
is much less amenable to interpretation as for two-view stereo, as the
...
long, perhaps it encountered a portion of the image where, due to noise
