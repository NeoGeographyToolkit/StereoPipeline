...
ways to do this (see using \texttt{ALIGNMENT\_METHOD}
...
Alternatively, it is possible to not have to define
...
opening both images in \texttt{qview} and comparing the coordinates of
...
just as you would in the configuration file. Due to technical
problems, we can however parse options that take multiple values that
are space delimited such as \texttt{corr-kernel} or
\texttt{corr-search}. For those options, please use a comma between
values. Below is an example of overriding the search range and
...
You should also know that when ever the \texttt{stereo} executable is
ran, it makes a copy of the configuration file used in
\textit{output-prefix}-stereo.default. Opening that output file will
show when the command was ran, what the flags were from the command
