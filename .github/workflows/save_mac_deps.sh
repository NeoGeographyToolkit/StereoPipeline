...
# See docs/building_asp.rst for the commands to build dependencies manually,
# if needed to understand failures when using conda.
...
# The tag set here must match the tag in build_test.sh. If
...
# to fetch the dependencies in build_test.sh.
tag=$1; shift 
...
# If making a new one, must make sure to update the tag in build_test.sh
